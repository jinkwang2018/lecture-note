...
public class boarddao {
	...
	//게시물 원본 글쓰기 * refer 생성하는 참조값을 만드는 함수를 만들어야한다.
	...
	//게시물 목록 구하기
		public List<board> list(int cpage , int pagesize){
			...
			try {
				...
				int start = cpage * pagesize - (pagesize -1)  ; //공식
				int end = cpage * pagesize;						//공식
				
				pstmt.setInt(1, end); // 5 rownum <= ?
				pstmt.setInt(2, start); // 1 where rn >= ?
				...
				while(rs.next()) {
					...
					list.add(b); // ArrayList에 add() pagesize = 5 >> 5건(board객체) add list에
	...
	public int reWriteOk(board boardata) {
			//content.jsp -> 답글 -> rewrite.jsp -> submit() -> rewriteok.jsp  
			//게시물 글쓰기(INSERT > 답글 ....) : refer ,step ,depth값을 변경
			//refer,step,depth를 변경하기 위해 내가 답글을 달려고 하는 글의 글번호가 필요하다. 
			...
			try {
				...
				//    원본글  답글  refer = 1 , step=1+1 > 2 , depth = 1
				//	     원본글  답글  refer = 1 , step=1 , depth = 1
				String step_update_sql = "update jspboard set step= step+1 where step  > ? and refer =? ";
				//위의 경우라면 "update jspboard set step= step+1 where step  > 0 and refer = 1 ";
	...
	public int deleteok(String idx , String pwd) throws SQLException {
		...
		//3.case : 원본글(답변이 없으면 삭제) 있으면 원본글만 삭제  정말 삭제!!
		...
		try {
				...
				//댓글 (reply 참조 관계) 삭제   cascade가 유용하다!! 
